...
interface CardIconsProps {
    children: React.ReactElement<LucideProps>; // Expect a Lucide icon component as children,
    asChild?: boolean;
}

function CardIcons({ children, asChild = false }: CardIconsProps) {
    return (
        <div className={`${asChild ? 'bg-none' : 'bg-primary rounded-md shadow-inner'} p-2 w-fit max-w-fit`}>
